      ...
      if (!autofetchNetworks) {
        if (ccdbTimestamp > 0) {
          ...
          if (retrieveSuccess) {
            ...
            network.initModel(networkPathLocally.value, enableNetworkOptimizations.value, strtoul(headers["Valid-From"].c_str(), NULL, 0), strtoul(headers["Valid-Until"].c_str(), NULL, 0));
            network.setActiveThreads(networkSetNumThreads.value);
          ...
          network.initModel(networkPathLocally.value, enableNetworkOptimizations.value);
          network.setActiveThreads(networkSetNumThreads.value);
